...
h_fig = figure;
h_3d = gca; % get current axis
axis equal % same len for data units along each axis
grid on
view(3);  % viewpoint of the observes (azimuth, elevation)
xlabel('x [m]'); ylabel('y [m]'); zlabel('z [m]')
quadcolors = lines(1); % Default RGB intensities [0 0.4470 0.7410]

set(gcf,'Renderer','OpenGL') % Set the property (renderer) of the object (current figure) to value (openGL)

%% *********************** INITIAL CONDITIONS ***********************
disp('Setting initial conditions...');
tstep    = 0.01; % this determines the time step at which the solution is given (100 Hz)
cstep    = 0.05; % image capture time interval (20 Hz)
max_iter = max_time/cstep; % max iteration (20 * 20 = 400)
nstep    = cstep/tstep; % ( = 5)
time     = 0; % current time
err = []; % runtime errors
